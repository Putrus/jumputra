...
namespace jp::game
{
   Wind::Wind(float impact, const math::Vector2<float>& maxVelocity,
      ...
      : graphics::Wind(sf::FloatRect(sf::Vector2f(rect.left, rect.top), sf::Vector2f(rect.width, rect.height))),
         logic::Wind(impact, maxVelocity, rect, acceleration, velocity) {}

   Wind::Wind(const nlohmann::json& json)
      : graphics::Wind(json), logic::Wind(json) {}
   ...
   {
      graphics::Wind::move(sf::Vector2f(mVelocity.x * dt, mVelocity.y * dt));
