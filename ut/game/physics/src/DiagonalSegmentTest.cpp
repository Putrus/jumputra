...
#include "../../../../game/physics/inc/DiagonalSegment.hpp"
...
namespace jp::ut::game::physics
{
   ...
   TEST_F(DiagonalSegmentTest, InitCorrect)
   {
      ...
      ASSERT_NO_THROW({ mSegment = new DiagonalSegment(a, b, SegmentSurface::Ordinary); });
   ...
   TEST_F(DiagonalSegmentTest, GetSurfaceSlippery)
   {
      math::Vector2<float> a = { 0.f, 0.f };
      math::Vector2<float> b = { 5.f, 5.f };
      ASSERT_NO_THROW({ mSegment = new DiagonalSegment(a, b, SegmentSurface::Slippery); });
      EXPECT_EQ(a, mSegment->a);
      EXPECT_EQ(b, mSegment->b);
   ...
   TEST_F(DiagonalSegmentTest, GetSurfaceSticky)
   {
      math::Vector2<float> a = { 0.f, 0.f };
      math::Vector2<float> b = { 5.f, 5.f };
      ASSERT_NO_THROW({ mSegment = new DiagonalSegment(a, b, SegmentSurface::Sticky); });
      EXPECT_EQ(a, mSegment->a);
      EXPECT_EQ(b, mSegment->b);
   ...
   {
      ASSERT_THROW({ mSegment = new DiagonalSegment({ 0.f, 0.f }, { 0.f, 5.f }, SegmentSurface::Ordinary); }, std::invalid_argument);
   }

   TEST_F(DiagonalSegmentTest, CheckCollisionSimpleLeft)
   {
      ASSERT_NO_THROW({ mSegment = new DiagonalSegment({ 100.f, 100.f }, { 200.f, 200.f }, SegmentSurface::Ordinary); });
   ...
   {
      ASSERT_NO_THROW({ mSegment = new DiagonalSegment({ 100.f, 100.f }, { 200.f, 0.f }, SegmentSurface::Ordinary); });
